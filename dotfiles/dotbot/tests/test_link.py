...
def test_link_canonicalization(home: str, dotfiles: Dotfiles, run_dotbot: Callable[..., None]) -> None:
    ...
    # Point to the config file in a symlinked EnvForgeKit directory.
    dotfiles_symlink = os.path.join(home, "EnvForgeKit-symlink")
...
def test_link_glob_4(home: str, dotfiles: Dotfiles, run_dotbot: Callable[..., None]) -> None:
    """Verify globbing works at the root of the home and EnvForgeKit directories."""
...
def test_link_no_canonicalize(key: str, home: str, dotfiles: Dotfiles, run_dotbot: Callable[..., None]) -> None:
    ...
    os.symlink(
        dotfiles.directory,
        os.path.join(home, "EnvForgeKit-symlink"),
        target_is_directory=True,
    )
    run_dotbot(
        "-c",
        os.path.join(home, "EnvForgeKit-symlink", os.path.basename(dotfiles.config_filename)),
        custom=True,
    )
    assert "EnvForgeKit-symlink" in os.readlink(os.path.join(home, ".f"))
...
def test_link_relative(home: str, dotfiles: Dotfiles, run_dotbot: Callable[..., None]) -> None:
    ...
    if sys.platform == "win32" and frel.startswith("\\\\?\\"):
        frel = frel[4:]
    assert frel == os.path.normpath("../../EnvForgeKit/f")
    ...
    assert nested_frel == os.path.normpath("../../f")

    d = os.readlink(os.path.join(home, ".d"))
    if sys.platform == "win32" and d.startswith("\\\\?\\"):
        d = d[4:]
    assert d == os.path.normpath("../../EnvForgeKit/d")
...
def test_source_is_not_overwritten_by_symlink_trickery(
    ...
    # Setup:
    #   *   A symlink exists from `~/.ssh` to `ssh` in the EnvForgeKit directory.
...
def test_link_defaults_1(home: str, dotfiles: Dotfiles, run_dotbot: Callable[..., None]) -> None:
    """Verify that link doesn't overwrite non-EnvForgeKit links by default."""
